...
var Template, colors, fs, idx, path, pkg, program, template, templates, util, _i, _len;
...
program.version(pkg.version).usage('[options] <file ...>').option('-o, --out [dir]', 'Output directory').option('-u, --uglify', 'Compress with ugilfy.js', true).option('-v, --verbose', 'Display building messages', true).option('-i, --include-function <function>', 'Including function name', 'include').parse(process.argv);
      ...
      return tpl.on('compiled', function() {
        if (program.out != null) {
          return fs.realpath(program.out, function(err, out) {
            ...
            return fs.writeFile(save, tpl.output, function(err) {
              if (err) {
